...
$(document).ready(function () {
  ...
  ********************************************************************/
  var client = new Usergrid.Client({
    orgName:'homeblabber', //your orgname goes here (not case sensitive)
    appName:'api', //your appname goes here (not case sensitive)
...
//bind the login button so we can send the user to facebook
  $('#btn-show-fb-login').bind('click', function() {
    //get the
    var apiKey = "124268704415421";
    ...
    link += "&redirect_uri=";
    link += location+path
    link += "&scope&response_type=token";
  ...
  client.getLoggedInUser(function(err, data, user) {
    if(err) {
      ...
      if (client.isLoggedIn()){
        appUser = user;
        var redirect = $.url().param("redirect_url");
        var loc = redirect + "#access_token=" + client.getToken();
        console.log("redirecting to app's redirect url");
    	window.location = loc ;
      }
    }
  });
      ...
      function (err,data) {
        if (err) {
          $('#login-section-error').html('There was an error logging you in.');
        	...
        	var loc = redirect + "#access_token=" + data.access_token;
        	window.location = loc ;
          }
        }
    );
